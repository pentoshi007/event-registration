            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
                {events.slice(0, 9).map((event) => (
                  <div key={event._id} onClick={() => setFeaturedEvent(event)} style={{ cursor: 'pointer' }}>
                    <EventCard event={event} onRegister={() => handleRegister(event._id)} showRegisterButton={false} />
                  </div>
                ))}
              </div>
          ...
          >
            <div className="bg-white/90 backdrop-blur-md rounded-2xl shadow-2xl overflow-hidden">
              ...
              <div className="bg-gradient-to-r from-blue-700 via-blue-600 to-blue-500 px-6 py-4">
                <h3 className="text-xl font-bold text-white text-center">Register for {selectedEvent.title}</h3>
              </div>
